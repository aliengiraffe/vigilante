...
func TestRunIssueSessionReconcilesExistingPullRequestAfterSuccessfulExit(t *testing.T) {
	...
	baseRunner := testutil.FakeRunner{
		Outputs: map[string]string{
			...
			"gh pr view --repo owner/repo 42 --json number,title,body,url,state,mergedAt,labels,isDraft,mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,baseRefName,headRefOid": `{"number":42,"title":"Demo","body":"Closes #7","url":"https://github.com/owner/repo/pull/42","state":"OPEN","mergedAt":null,"labels":[],"isDraft":false,"mergeable":"MERGEABLE","mergeStateStatus":"CLEAN","reviewDecision":"","statusCheckRollup":[],"baseRefName":"main"}`,
		},
	}
	store := state.NewStore()
	if err := store.EnsureLayout(); err != nil {
...
func TestRunIssueSessionLeavesIncompleteWhenBranchLookupFindsClosedPullRequest(t *testing.T) {
	...
	baseRunner := testutil.FakeRunner{
		Outputs: map[string]string{
			...
			"gh pr view --repo owner/repo 42 --json number,title,body,url,state,mergedAt,labels,isDraft,mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,baseRefName,headRefOid": `{"number":42,"title":"Demo","body":"Closes #7","url":"https://github.com/owner/repo/pull/42","state":"CLOSED","mergedAt":null,"labels":[],"isDraft":false,"mergeable":"UNKNOWN","mergeStateStatus":"UNKNOWN","reviewDecision":"","statusCheckRollup":[],"baseRefName":"main"}`,
