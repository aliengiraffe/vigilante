...
func RunIssueSession(ctx context.Context, env *environment.Environment, store *state.Store, it backend.IssueTracker, target state.WatchTarget, issue backend.WorkItem, session state.Session) state.Session {
	if session.Repo == "" {
	...
	if err := it.PostComment(ctx, target.Repo, issue.Number, startBody); err != nil {
		session.Status = state.SessionStatusFailed
		session.IterationInProgress = false
		session.LastError = err.Error()
		session.EndedAt = time.Now().UTC().Format(time.RFC3339)
	...
	if err != nil {
		...
		_ = it.PostComment(ctx, target.Repo, issue.Number, body)
		return session
	}
...
func RunConflictResolutionSession(ctx context.Context, env *environment.Environment, store *state.Store, it backend.IssueTracker, target state.WatchTarget, session state.Session, pr backend.PullRequest) error {
	...
	if err != nil {
		...
		_ = it.PostComment(ctx, target.Repo, session.IssueNumber, body)
		return err
	}
...
func RunCIRemediationSession(ctx context.Context, env *environment.Environment, store *state.Store, it backend.IssueTracker, target state.WatchTarget, session state.Session, pr backend.PullRequest, failingChecks []backend.StatusCheck) error {
	repoSlug := session.Repo
	if repoSlug == "" {
		repoSlug = target.Repo
	}
