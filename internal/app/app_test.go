...
func TestScanOnceWithMaxParallelOnePreservesSerialBehavior(t *testing.T) {
	...
	app.env.Runner = testutil.FakeRunner{
		...
		Outputs: map[string]string{
			...
			"git worktree prune": "ok",
			"git worktree add -b vigilante/issue-1 " + worktreePath1 + " main":                                                                   "ok",
			sessionStartCommentCommand("owner/repo", 1, worktreePath1, "vigilante/issue-1"):                                                      "ok",
			issuePromptCommand(worktreePath1, "owner/repo", repoPath, 1, "first", "https://github.com/owner/repo/issues/1", "vigilante/issue-1"): "done",
		},
	}
	if err := app.state.EnsureLayout(); err != nil {
		t.Fatal(err)
...
func TestScanOnceStartsMultipleIssuesUpToConfiguredLimit(t *testing.T) {
	...
	app.env.Runner = testutil.FakeRunner{
		...
		Outputs: map[string]string{
			...
			sessionStartCommentCommand("owner/repo", 1, worktreePath1, "vigilante/issue-1"):                                                       "ok",
			sessionStartCommentCommand("owner/repo", 2, worktreePath2, "vigilante/issue-2"):                                                       "ok",
			issuePromptCommand(worktreePath1, "owner/repo", repoPath, 1, "first", "https://github.com/owner/repo/issues/1", "vigilante/issue-1"):  "done",
			issuePromptCommand(worktreePath2, "owner/repo", repoPath, 2, "second", "https://github.com/owner/repo/issues/2", "vigilante/issue-2"): "done",
		},
	}
	if err := app.state.EnsureLayout(); err != nil {
		t.Fatal(err)
...
func TestScanOnceEnforcesLimitsIndependentlyAcrossRepositories(t *testing.T) {
	...
	app.env.Runner = testutil.FakeRunner{
		...
		Outputs: map[string]string{
			...
			"git worktree prune": "ok",
			"git worktree add -b vigilante/issue-1 " + worktreeA1 + " main":                                                                              "ok",
			"git worktree add -b vigilante/issue-2 " + worktreeA2 + " main":                                                                              "ok",
			"git worktree add -b vigilante/issue-10 " + worktreeB10 + " main":                                                                            "ok",
			sessionStartCommentCommand("owner/repo-a", 1, worktreeA1, "vigilante/issue-1"):                                                               "ok",
			sessionStartCommentCommand("owner/repo-a", 2, worktreeA2, "vigilante/issue-2"):                                                               "ok",
			sessionStartCommentCommand("owner/repo-b", 10, worktreeB10, "vigilante/issue-10"):                                                            "ok",
			issuePromptCommand(worktreeA1, "owner/repo-a", repoPathA, 1, "first-a", "https://github.com/owner/repo-a/issues/1", "vigilante/issue-1"):     "done",
			issuePromptCommand(worktreeA2, "owner/repo-a", repoPathA, 2, "second-a", "https://github.com/owner/repo-a/issues/2", "vigilante/issue-2"):    "done",
			issuePromptCommand(worktreeB10, "owner/repo-b", repoPathB, 10, "first-b", "https://github.com/owner/repo-b/issues/10", "vigilante/issue-10"): "done",
		},
	}
	if err := app.state.EnsureLayout(); err != nil {
		t.Fatal(err)
